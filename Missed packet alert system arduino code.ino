...
const int EEPROM_P_IN_ADDRESS = 0;                                    
...
const int STATE_P_IN = 3;
const int STATE_P_OUT = 4;
const int STATE_P_BUF = 5;
const int STATE_SETTINGS_2 = 6;
const int STATE_TIME = 7;
const int STATE_AUDIO = 8;
const int STATE_OUTLET = 9;

const int LAST_STATE = 9;         // Index of last state in state machine (UPDATE IF STATES ARE ADDED OR REMOVED!!!)
...
void loop() {
  ...
  if (thisMainLoopPressureSampleTime - lastMainLoopPressureSampleTime > PRESSURE_SAMPLE_TIME) {     // runs code in conditional every PRESSURE_SAMPLE_TIME milliseconds
    ...
    if (PRESSURE_INNER > MAX_PRESSURE_NO_VACUUM) {  // if vacuum is higher than atmospheric
    ...
    if (PRESSURE_OUTER > MAX_PRESSURE_NO_VACUUM) {  // if vacuum is higher than atmospheric
    ...
    if (PRESSURE_INNER <= MAX_PRESSURE_NO_VACUUM && PRESSURE_OUTER <= MAX_PRESSURE_NO_VACUUM) {   // Once both vacuums shut off
  ...
  if (ROTATION) {                                                                   // If a rotation (+1 or -1 has been flagged)                                                
    if (STATE == STATE_P_IN) {                                                      //  If the inner pressure limit is currently editable
...
void printVariable(int state) {
  ...
  int home_row = 0;
  int size = 0;
  if (state == STATE_P_IN) {                      // If the inner pressure limit can be changed:
    home_col = 16;
    home_row = 1;
