...
---> const int MIN_LOWER_PRESSURE_GAP = 4;		  // Minimum acceptable difference in pressure between baseline and limit [%]
---> const int WARNING_ALERT_DURATION = 5000;	  // Duration to display warning alerts [ms]
---> const int MISSED_PACK_ALERT_DURATION = 3000;  // Duration to display pack missed alert [ms]

---> const ALERT_MSGS[][][] = {					  // Alert messages split into three lines of text with max 20 chars each
--->   {"    Inner sensor", " baselines & limits", "   are too close"},
--->   {"    Outer sensor", " baselines & limits", "   are too close"},
--->   {"", "   Packed missed!", ""}  
---> }

///////////////     ALERT CODES 		/////////////////////////////////////////////
---> const int ALERT_BASELINE_INNER = 0;	  // Alert code for when the inner P baseline is too close to a P limit (index of ALERT_MSGS)
---> const int ALERT_BASELINE_OUTER = 1;	  // Alert code for when the outer P baseline is too close to a P limit (index of ALERT_MSGS)
---> const int ALERT_MISSED_PACK = 2;         // Alert code for when a missed packet is detected (index of ALERT_MSGS)
...
---> int ALERT_ACTIVE = 0;						  // Stores whether there is an active or pending alert to display
---> int ALERT_CODE = 0; 						  // Stores code of alert to display
---> int ALERT_DUR = 0;							  // Stores duration for which to display alert [ms]
---> int ALERT_RETURN_STATE = STATE_HOME; 	 	  // Stores state of state machine to go to after alert is done displaying
...
--->   static unsigned long alertStartTime = 0;							  // init timer for alert state
  
--->   // Check to see if an alert needs to be displayed or removed
--->   if (ALERT_ACTIVE && NEXT_STATE != STATE_ALERT) {					  // If an alert flag is raised but has not yet been displayed
--->     alertStartTime = millis();										  //  Save current time as start time of alert
---> 	NEXT_STATE = STATE_ALERT; 										  //  Set next state to the alert state
--->   } else if (ALERT_ACTIVE) { 										  // If an alert is currently being displayed
--->     if (millis() - alertStartTime >= ALERT_DUR) {                     //  If it has been displayed for long enough
---> 	  NEXT_STATE = ALERT_RETURN_STATE; 								  //   Set next state to a non-alert state to go back to
---> 	  ALERT_ACTIVE = 0; 											  //   Lower the alert flag. 
---> 	}
--->   }
...
--->     // Check to make sure Pressure Baselines and Pressure limits are far enough apart to differentiate. If not, display warning alert message.
---> 	if ((PRESSURE_INNER_LIMIT - PRESSURE_INNER_BASELINE) / 2. < MIN_LOWER_PRESSURE_GAP) {
---> 	  raiseAlert(ALERT_BASELINE_INNER, WARNING_ALERT_DURATION, STATE_HOME);
---> 	} else if ((PRESSURE_OUTER_LIMIT - PRESSURE_OUTER_BASELINE) / 2. < MIN_LOWER_PRESSURE_GAP) {
---> 	  raiseAlert(ALERT_BASELINE_OUTER, WARNING_ALERT_DURATION, STATE_HOME);
---> 	}
    ...
    float maxPressureNoVacuumInner = PRESSURE_INNER_BASELINE + ((PRESSURE_INNER_LIMIT - PRESSURE_INNER_BASELINE) / 2.);
    float maxPressureNoVacuumOuter = PRESSURE_OUTER_BASELINE + ((PRESSURE_OUTER_LIMIT - PRESSURE_OUTER_BASELINE) / 2.);
...
--->      if (STATE != STATE_ALERT && NEXT_STATE != STATE_ALERT) {  // and if the state machine is not displaying an alert
        NEXT_STATE++;         //  go to the next state
...
--->   raiseAlert(ALERT_MISSED_PACK, MISSED_PACK_ALERT_DURATION, STATE_HOME);  // display alert that a missed pack has been detected
...
---> ///////////////////////////////////////////////
---> //
---> // fn raiseAlert(alert_code, dur, ret_state)
---> // Takes in:
---> //	 - an alert code (see "ALERT CODES" section in globals)
---> //   - a duration, dur, in milliseconds, that the alert will display
---> //   - a state, ret_state, which will display once the alert has finished
---> // Pushes these arguments into global vars and sets the ALERT_ACTIVE flag
---> //
---> /////////////////////////////////////////////////////////////////////////////////////////
---> 
---> void raiseAlert(int alert_code, int dur, int ret_state) {
--->   ALERT_CODE = alert_code;
--->   ALERT_DUR = dur;
--->   ALERT_RETURN_STATE = ret_state;
--->   ALERT_ACTIVE = 1;
---> }
...
void displayMessageToLCD(int state) {
  ...
  if (state == STATE_INIT) {                // on the init state
    ...
    delay(3000);                            //  and hold it for a bit
--->   } else if (state == STATE_ALERT) {        // on the alert state
--->     displayAlert();                         // display the alert message
--->     lcd.noBlink();                          // disable blinking cursor
...
--->   lcd.clear();
--->   lcd.print("     * ALERT! *");
--->   for (int i=1; i<=3; i++) {  		// loop through rows 1, 2, and 3 of LCD screen
--->     lcd.setCursor(0,i);				//  printing the appropriate message
---> 	 lcd.print(ALERT_MSGS[ALERT_CODE][i])
--->   }
---> }
